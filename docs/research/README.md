...
**LOC-28 — вопросы организаторам.** Одно сообщение в Q&A-канал: (1) метрика — MAE/RMSE, нормировка, per turbine или сумма; (2) таймзона выходных меток (часы SCADA UTC+6 / UTC / Astana UTC+5); (3) что значит «прогноз на 31 января» — момент выпуска, границы 48 ч, засчитывается ли внутридневной пересчёт или только выпуск до начала суток; (4) формат — один ряд на весь февраль или файл на каждый выпуск, нормализованные единицы или МВт·ч, по турбинам или сумма; (5) как считаются часы без SCADA-данных; (6) допустим ли Open-Meteo как «открытый источник» или ожидается сырой ECMWF/GFS; (7) почасовая выработка = среднее шести 10-мин значений с меткой начала часа, как считать часы с <6 сэмплами. Ответа не ждём: дефолты (часы SCADA, MAE → P50, per turbine, dayahead 18:00 UTC D−1, 48 ч с 00:00 D+1 SCADA, среднее ≥4 сэмплов с меткой начала часа, per-issue + hourly CSV) записаны в `docs/research/questions.md` и `docs/assumptions.md`.
