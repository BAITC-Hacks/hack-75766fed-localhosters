...
`Clock` и адаптеры данных обеспечивают доступность источника и схему. Числовой прогноз считает модель. Планировщик выбирает, публиковать ли результат, нужен ли новый выпуск и как объяснить решение. Выбор планировщика проверяется: нельзя использовать другой ран или заявить коррекцию, которой не было в числовом пути. Scripted-режим воспроизводим без API-ключа. В `openai` режиме Pydantic AI получает валидированный контекст и три read-only инструмента (`get_quality`, `get_available_run`, `get_revision_context`). Переход на более широкий tool loop после интеграции модели и SCADA.
...
ADR: для структурированного `IssueDecision`, `TestModel` и лимита запросов выбран Pydantic AI. Провайдер — OpenAI (`OPENAI_MODEL`, по умолчанию `gpt-5.4-mini`); Pydantic AI провайдер-независим, смена модели — одна переменная окружения. Голый SDK требует собственного цикла валидации/ретраев; LangGraph/CrewAI/Agents SDK добавляют стоимость интеграции для этого короткого процесса. Живой прогон на всём феврале (29 выпусков + 29 пересчётов, `make llm-replay`): 58 из 58 решений LLM совпали с детерминированными правилами, 0 откатов, ~1.2 тыс. входных и ~160 выходных токенов на решение, $0.095 за весь месяц — [отчёт](../reports/llm_replay_test.md), прогоны в `runs/llm/test/`. Если LLM недоступен или ссылается на ран, не прошедший проверку доступности, выпуск принимается по правилам, причина пишется в `fallback_reason` трейса и `decision.json` (`tests/test_llm_fallback.py`).
