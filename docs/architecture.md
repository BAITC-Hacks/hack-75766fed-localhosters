...
ADR: для структурированного `IssueDecision`, `TestModel` и лимита запросов выбран Pydantic AI. Провайдер — OpenAI (`OPENAI_MODEL`, по умолчанию `gpt-5-mini`); Pydantic AI провайдер-независим, смена модели — одна переменная окружения. Голый SDK требует собственного цикла валидации/ретраев; LangGraph/CrewAI/Agents SDK добавляют стоимость интеграции для этого короткого процесса. Цена 58 выпусков будет рассчитана по реальному usage после запуска с ключом.
